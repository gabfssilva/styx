...
import org.styx.handler.EventHandler.{UnsuccessfullyHandled, SuccessfullyHandled}
      ...
      e canApply actualState match {
        case Valid(true, _) =>
          ...
          execute(e)(state)
            .flatMap { _ =>
              eventHandler
                .handle(actualState.aggregationId, e, state)
                .map {
                  case SuccessfullyHandled(_) => execute(e)(state); state
                  case UnsuccessfullyHandled(ex, _) => throw InvalidExecutionException(s"there was a problem to insert the event into the event store: ${ex.getMessage}")
                }
            }
